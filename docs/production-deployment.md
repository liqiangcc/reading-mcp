...
Create a rollback checkpoint before changing the active link. The deployment script verifies the package checksum and manifest, verifies the packaged binary checksum, preserves canonical state, retains the known-good binary, atomically switches the `reading-mcp` symlink, and restarts systemd.
...
- malformed version/SHA identities;
