...
此前向 Coordinator 提出的 F11 区域定义问题（comment `5647122033`）
不应继续作为全局开发阻塞：重新读取冻结 gold，两个对象的要求明确是
`coarse_or_visual`，没有逐像素覆盖标注框内全部空白的要求。继续证明两个
真实对象均保留为粗粒度/视觉内容并绑定原页，不用 gold 反推生产框，也不
新增全空白覆盖门槛。最终正式 Rust/MCP 发布、原页回看及来源顺序仍须验收；
诊断成功不代替这些证据。
