...
def main():
    ...
    for case in ("F02","F06","F07","F08","F14"):
        ...
        try:
            ...
            if case == "F08":
                expected="\n\n".join(p["text"] for p in sorted([p for p in gold["paragraphs"] if p["font"]=="goldeng"], key=lambda p:p["id"]))
            ...
            if out[case]["cer"]["rate"] is None or out[case]["cer"]["rate"] > out[case]["thresholds"]["cer"] or ambiguous or (wer is not None and wer["rate"] > out[case]["thresholds"]["wer"]): failures.append(case)
        except Exception as error:
            out[case]={"error":str(error),"stderr":getattr(locals().get("proc",None),"stderr",b"").decode(errors="replace")[-4096:]}; failures.append(case)
